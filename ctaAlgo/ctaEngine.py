...
class CtaEngine(object):
    ...
    def processTradeEvent(self, event):
        ...
        if trade.vtOrderID in self.orderStrategyDict:
            ...
            # 计算策略持仓
            # by lm  支持单策略多合约情况，保存各个合约的持仓和总持仓
            if trade.vtSymbol not in strategy.pos:  # 保存新合约
    ...
    def loadBar(self, dbName, collectionName, days):
        ...
        if barData:
            ...
            self.writeCtaLog(u'数据库%s第%s日Bar数据不存在。' % (dbName, days))
            print u'数据库%s第%s日Bar数据不存在。' % (dbName, days)
        return l
    ...
    def loadStrategy(self, setting):
        ...
        if name in self.strategyDict:
            ...
            # 创建策略实例
            strategy = strategyClass(self, setting)    # 初始化策略，调用策略类里的__init__(self, ctaEngine, setting)函数
            self.strategyDict[name] = strategy
            
            # 保存Tick映射关系
            # by lm 支持单个策略订阅单个或多个合约
            vtSymbolset = setting['vtSymbol']
            vtSymbolList = vtSymbolset.split(',')  # 多个合约用","分开，配置文件中"vtSymbol": "IF1602,IF1603",单个合约保持不变
